...
describe('task_context auto_safe block', () => {
  const validBody = `# TASK-1: example
...
`
  ...
  function makeContextSvc(
    task: Task
  ): Parameters<typeof taskTools.register>[1] {
    return {
      getTask: (id: string): Task | null => (id === task.id ? task : null),
      getDependencies: () => [],
      getSubtasks: () => [],
  ...
  function getTaskContext(tools: CapturedTool[]): CapturedTool {
    const t = tools.find((x) => x.name === 'task_context')
    if (!t) throw new Error('task_context not registered')
    return t
  }

  interface TextResponseShape {
    content: Array<{ type: string; text: string }>
  }

  function parseAutoSafe(res: unknown): {
    labeled: boolean
    valid: boolean
    errors: string[]
    suggestions: string[]
  } {
    const text = (res as TextResponseShape).content[0].text
    const parsed = JSON.parse(text) as { auto_safe: ReturnType<typeof parseAutoSafe> }
    return parsed.auto_safe
  }

  it('reports valid + labeled when body passes and auto-safe label is present', async () => {
    const { server, tools } = makeFakeServer()
    const task = makeTask('READY', { body: validBody, labels: ['auto-safe'] })
    taskTools.register(server, makeContextSvc(task))

    const res = await getTaskContext(tools).cb({ id: task.id })
    const auto_safe = parseAutoSafe(res)

    expect(auto_safe.labeled).toBe(true)
    expect(auto_safe.valid).toBe(true)
    expect(auto_safe.errors).toEqual([])
    expect(auto_safe.suggestions).toEqual([])
  })

  it('reports valid + unlabeled when body passes but label is missing', async () => {
    const { server, tools } = makeFakeServer()
    const task = makeTask('READY', { body: validBody, labels: [] })
    taskTools.register(server, makeContextSvc(task))

    const res = await getTaskContext(tools).cb({ id: task.id })
    const auto_safe = parseAutoSafe(res)

    expect(auto_safe.labeled).toBe(false)
    expect(auto_safe.valid).toBe(true)
    expect(auto_safe.errors).toEqual([])
  })

  it('reports invalid + unlabeled when body is missing ## Scope', async () => {
    const { server, tools } = makeFakeServer()
    const body = validBody.replace(/## Scope[\s\S]*$/, '')
    const task = makeTask('READY', { body, labels: [] })
    taskTools.register(server, makeContextSvc(task))
    ...
    expect(auto_safe.labeled).toBe(false)
    expect(auto_safe.valid).toBe(false)
    expect(auto_safe.errors).toContain('Missing ## Scope section')
    expect(auto_safe.suggestions.some((s) => /add a `## Scope` section/i.test(s))).toBe(true)
    expect(auto_safe.errors.length).toBe(auto_safe.suggestions.length)
  })

  it('reports invalid + labeled (drift case) — labeled but body now fails', async () => {
    const { server, tools } = makeFakeServer()
    const body = validBody.replace(/## Scope[\s\S]*$/, '')
    const task = makeTask('READY', { body, labels: ['auto-safe'] })
    taskTools.register(server, makeContextSvc(task))
    ...
    expect(auto_safe.labeled).toBe(true)
    expect(auto_safe.valid).toBe(false)
    expect(auto_safe.errors.length).toBeGreaterThan(0)
    expect(auto_safe.suggestions.length).toBe(auto_safe.errors.length)
  })
})
