...
import type {
  Phase, Task, Session, Conversation, ContextSource, DerivedProgress
} from '../task-types'
...
export function buildProjectContext(
  ...
  return {
    ...
    contextSources: sources.map(s => ({ label: s.label, category: s.category, sourcePath: s.sourcePath }))
  }
}

function fetchProject(svc: SqliteTaskService, projectId: string): ProjectContextBundle['project'] | null {
  return svc.getProject(projectId)
}

function pickActivePhase(svc: SqliteTaskService, projectId: string): ProjectContextBundle['currentState']['activePhase'] {
  const phases = svc.findPhases(projectId)
  const active = phases.find((p: Phase) => p.startDate && !p.completedDate) ?? phases.find((p: Phase) => p.status === 'open') ?? null
  if (!active) return null
  return { id: active.id, title: active.title, progress: svc.getPhaseProgress(active.id) }
}

function pickActiveTasks(svc: SqliteTaskService, projectId: string): Array<Pick<Task, 'id' | 'title' | 'status' | 'priority'>> {
...
function pickLastSession(svc: SqliteTaskService, projectId: string): ProjectContextBundle['currentState']['lastSession'] {
...
function pickOpenConversations(svc: SqliteTaskService, projectId: string): ProjectContextBundle['currentState']['openConversations'] {
  const open = [
...
function loadFileSource(
  ...
  const match = sources.find(s => s.sourceType === 'file' && s.category === category && matcher.test(s.label))
  if (!match) return null
  return readMarkdown(contentRoot, match.sourcePath, depth)
}

function loadConventions(sources: ContextSource[], contentRoot: string, depth: ProjectContextDepth): string | null {
  const convs = sources.filter(s =>
    s.sourceType === 'file' && s.category === 'how' && !/architecture/i.test(s.label)
  )
...
function readMarkdown(contentRoot: string, sourcePath: string, depth: ProjectContextDepth): string | null {
