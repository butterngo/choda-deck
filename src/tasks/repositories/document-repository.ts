...
export class DocumentRepository {
  ...
  findByProject(projectId: string, type?: DocumentType): Document[] {
    const rows = type
      ? this.db.prepare('SELECT * FROM documents WHERE project_id = ? AND type = ? ORDER BY created_at').all(projectId, type) as Array<Record<string, unknown>>
      : this.db.prepare('SELECT * FROM documents WHERE project_id = ? ORDER BY type, created_at').all(projectId) as Array<Record<string, unknown>>
