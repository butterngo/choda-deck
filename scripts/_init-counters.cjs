...
function initSchema(db2) {
  createCoreTables(db2);
  runLegacyMigrations(db2);
  createM1Tables(db2);
  createIndexes(db2);
}
function createCoreTables(db2) {
  db2.exec(`
...
function runLegacyMigrations(db2) {
  try {
    db2.exec("ALTER TABLE phases ADD COLUMN start_date TEXT");
  } catch {
  }
  try {
    db2.exec("ALTER TABLE phases ADD COLUMN completed_date TEXT");
  } catch {
  }
  try {
    db2.exec("ALTER TABLE tasks ADD COLUMN feature_id TEXT");
  } catch {
  }
  try {
    db2.exec(`
  ...
  db2.exec("DROP TABLE IF EXISTS epics");
  db2.exec("DROP TABLE IF EXISTS task_dependencies");
  try {
    db2.exec("DROP INDEX IF EXISTS idx_tasks_epic");
  } catch {
  }
  try {
    db2.exec("ALTER TABLE conversations ADD COLUMN created_by TEXT NOT NULL DEFAULT ''");
  } catch {
  }
  try {
    db2.exec("ALTER TABLE conversations ADD COLUMN decided_at TEXT");
  } catch {
  }
  try {
    db2.exec("ALTER TABLE sessions ADD COLUMN workspace_id TEXT");
  } catch {
  }
  try {
    db2.exec("ALTER TABLE sessions ADD COLUMN task_id TEXT");
  } catch {
  }
  try {
    db2.exec("ALTER TABLE tasks ADD COLUMN phase_id TEXT");
  } catch {
  }
  try {
    db2.exec("ALTER TABLE tasks ADD COLUMN body TEXT");
  } catch {
  }
  db2.exec(`
  ...
  migrateConversationMessages(db2);
}
function migrateConversationMessages(db2) {
  const cols = db2.pragma("table_info(conversation_messages)");
  const colNames = cols.map((c) => c.name);
  if (colNames.includes("author") && !colNames.includes("author_name")) {
    db2.exec("ALTER TABLE conversation_messages RENAME COLUMN author TO author_name");
  }
  if (!colNames.includes("metadata_json")) {
    try {
      db2.exec("ALTER TABLE conversation_messages ADD COLUMN metadata_json TEXT");
    } catch {
    }
  }
}
function createM1Tables(db2) {
  db2.exec(`
...
function createIndexes(db2) {
  db2.exec("CREATE INDEX IF NOT EXISTS idx_phases_project ON phases(project_id)");
  db2.exec("CREATE INDEX IF NOT EXISTS idx_features_project ON features(project_id)");
  db2.exec("CREATE INDEX IF NOT EXISTS idx_features_phase ON features(phase_id)");
  db2.exec("CREATE INDEX IF NOT EXISTS idx_tasks_project ON tasks(project_id)");
  db2.exec("CREATE INDEX IF NOT EXISTS idx_tasks_status ON tasks(project_id, status)");
  db2.exec("CREATE INDEX IF NOT EXISTS idx_tasks_feature ON tasks(feature_id)");
  db2.exec("CREATE INDEX IF NOT EXISTS idx_tasks_phase ON tasks(phase_id)");
  db2.exec("CREATE INDEX IF NOT EXISTS idx_tasks_parent ON tasks(parent_task_id)");
  db2.exec("CREATE INDEX IF NOT EXISTS idx_tags_item ON tags(item_id)");
  db2.exec("CREATE INDEX IF NOT EXISTS idx_relationships_from ON relationships(from_id)");
  db2.exec("CREATE INDEX IF NOT EXISTS idx_relationships_to ON relationships(to_id)");
  db2.exec("CREATE INDEX IF NOT EXISTS idx_documents_project ON documents(project_id)");
  db2.exec("CREATE INDEX IF NOT EXISTS idx_sessions_project ON sessions(project_id)");
  db2.exec("CREATE INDEX IF NOT EXISTS idx_sessions_status ON sessions(project_id, status)");
  db2.exec("CREATE INDEX IF NOT EXISTS idx_context_sources_project ON context_sources(project_id)");
  db2.exec("CREATE INDEX IF NOT EXISTS idx_conversations_project ON conversations(project_id)");
  db2.exec(
    "CREATE INDEX IF NOT EXISTS idx_conv_messages_conv ON conversation_messages(conversation_id)"
  );
  db2.exec("CREATE INDEX IF NOT EXISTS idx_conv_links_conv ON conversation_links(conversation_id)");
  db2.exec(
...
var ProjectRepository = class {
  ...
  addWorkspace(projectId, id, label, cwd) {
    this.db.prepare("INSERT OR REPLACE INTO workspaces (id, project_id, label, cwd) VALUES (?, ?, ?, ?)").run(id, projectId, label, cwd);
    return { id, projectId, label, cwd };
  }
  getWorkspace(id) {
    const row = this.db.prepare("SELECT id, project_id, label, cwd FROM workspaces WHERE id = ?").get(id);
    if (!row) return null;
    return {
      id: row.id,
...
var RelationshipRepository = class {
  ...
  getForItem(itemId) {
    const rows = this.db.prepare("SELECT * FROM relationships WHERE from_id = ? OR to_id = ?").all(itemId, itemId);
...
var SessionRepository = class {
  ...
  findByProject(projectId, status) {
    ...
    ).all(projectId, status) : this.db.prepare("SELECT * FROM sessions WHERE project_id = ? ORDER BY started_at DESC").all(projectId);
...
var ConversationRepository = class {
  ...
  findByProject(projectId, status) {
    ...
    ).all(projectId, status) : this.db.prepare("SELECT * FROM conversations WHERE project_id = ? ORDER BY created_at DESC").all(projectId);
  ...
  getLinks(conversationId) {
    const rows = this.db.prepare("SELECT * FROM conversation_links WHERE conversation_id = ?").all(conversationId);
    return rows.map((r) => ({
