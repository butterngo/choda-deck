...
var TaskRepository = class {
  ...
  nextTaskId(projectId) {
    const rows = this.db.prepare("SELECT id FROM tasks WHERE project_id = ? AND id GLOB 'TASK-[0-9]*'").all(projectId);
    let max = 0;
