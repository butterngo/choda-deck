...
- Site: `src/core/executor/coder.ts` (`ClaudePCoderDriver`, `input.systemPrompt` assembly L66 area)
...
- **Không port sang FE Playwright pilot** — coder.ts prompt share giữa queue + pilot, prompt change áp dụng cả 2 (intended), nhưng smoke harness scope FE pilot defer.
